...
function isValidFunction(expression: CallExpression) {
  if (!isIdentifier(expression.callee)) return false;
...
function isValidExpression(expression: Expression): boolean {
  if (isLiteral(expression)) {
    ...
    else throw new Error(`Invalid variable ${expression.name}`);
  } else if (isCallExpression(expression)) {
    return (
      isValidFunction(expression) &&
...
function construcBinaryExpression(expression: BinaryExpression) {
  const operatorMap = {
    "+": "$add",
    "-": "$subtract",
    "*": "$multiply",
...
function constructCallExpression(expression: CallExpression) {
  if (!isIdentifier(expression.callee)) throw new Error();
  switch (expression.callee.name) {
    case "exp":
...
function constructExpression(expression: Expression): any {
  if (isLiteral(expression)) {
...
function extractVariables(expression: Expression): string[] {
...
const expression =
  "sharedCount + ( commentsCount / 3 ) - exp_decay(createdAt, 1, 1)";
